#!/bin/bash
# Push wireguard container to registry
...
main() {
    print_push "Pushing wireguard container to registry..."
    ...
    if ! docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "^${TAG}$"; then
        ...
        print_status "Build it first with: ./deploy/containers/build-wireguard.sh"
        exit 1
    fi
