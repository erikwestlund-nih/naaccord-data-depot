#!/bin/bash
# Build nginx container only (fast, ~1 minute)
...
main() {
    print_build "Building nginx container (reverse proxy)..."
    print_status "This is a small container and builds quickly (~1 minute)"
    print_status "Registry: ${REGISTRY}/nginx:latest"
    ...
    if ${BUILD_CMD} \
        ...
        -f deploy/containers/nginx/Dockerfile \
        deploy/containers/nginx/ 2>&1 | tee "build-nginx.log"; then

        print_status "✓ Nginx container built successfully"

        # Show image info
        echo ""
        print_status "Image details:"
        docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}\t{{.Size}}" | grep nginx
        ...
        echo "  docker run --rm -p 8080:80 ${REGISTRY}/nginx:latest"
    else
        print_error "✗ Nginx container build failed"
        print_error "Check build-nginx.log for details"
        exit 1
    fi
}
