#!/bin/bash
# Push services container to registry
...
main() {
    print_push "Pushing services container to registry..."
    ...
    if ! docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "^${TAG}$"; then
        ...
        print_status "Build it first with: ./deploy/containers/build-services.sh"
        exit 1
    fi
